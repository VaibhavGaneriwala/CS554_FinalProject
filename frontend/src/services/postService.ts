...
export const postService = {
    ...
    getPosts: async (userId?: string, type?: string, page = 1, limit = 20) => {
        ...
        const response = await api.get<ApiResponse<{posts: Post[]; pagination: Pagination;}>>('/posts', { params });
        return response.data;
    },
    ...
    likePost: async (postId: string): Promise<ApiResponse<{ liked: boolean; likesCount: number }>> => {
        const response = await api.post<ApiResponse<{ liked: boolean; likesCount: number }>>(`/posts/${postId}/like`);
        return response.data;
    },
    addComment: async (postId: string, text: string): Promise<ApiResponse<Comment>> => {
        const response = await api.post<ApiResponse<Comment>>(`/posts/${postId}/comment`, { text });
        return response.data;
    },
